<?php
/**
 * Single Template: Industry
 * Displays a single Industry custom post type entry
 *
 * @package skillplus
 */
get_header();
...
<section class="relative bg-gradient-to-br from-gray-50 dark:from-zinc-950 via-emerald-50 dark:via-black to-teal-50 dark:to-zinc-950 pt-32 pb-16 lg:pt-44 lg:pb-20 overflow-hidden">

    <div class="hidden lg:block absolute inset-0 pointer-events-none">
        <div class="absolute left-[10%] top-[15%] w-[280px] h-[280px] rounded-full bg-emerald-200 dark:bg-emerald-900 opacity-90 blur-[60px]"></div>
        <div class="absolute right-[12%] top-[30%] w-[200px] h-[200px] rounded-full bg-teal-300 dark:bg-teal-950 opacity-80 blur-[40px]"></div>
        <div class="absolute inset-0 bg-gradient-to-br from-emerald-400/10 via-emerald-500/10 to-emerald-600/10"></div>
    </div>
    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="max-w-4xl mx-auto text-center">
            ...
            <div class="inline-flex items-center justify-center mb-5">
                <a href="<?php echo esc_url($cat_link); ?>" class="py-1 px-3 text-emerald-600 font-semibold border-b-2 border-emerald-600 hover:text-emerald-700 transition-colors">
            ...
            <div class="flex items-center justify-center mb-6">
                <div class="size-16 rounded-full bg-emerald-100/40 dark:bg-emerald-950/40 flex items-center justify-center">
                    <?php if ($industry_icon): ?>
                        <i data-lucide="<?php echo esc_attr($industry_icon); ?>"
                           class="w-8 h-8 text-emerald-600 dark:text-emerald-400"></i>
                    <?php else: ?>
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-600 dark:text-emerald-400" aria-hidden="true">
            ...
            <!-- Title -->
            <h1 class="font-black text-3xl lg:text-5xl font-bold mb-6 leading-[1.2] bg-linear-to-r from-emerald-900 via-teal-900 to-emerald-900 dark:from-gray-50 dark:via-emerald-300 dark:to-teal-900 bg-clip-text text-transparent">
                <?php the_title(); ?>
            </h1>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mt-12 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <?php
                    $prev_post = get_previous_post(true, '', 'industry_category');
                    $next_post = get_next_post(true, '', 'industry_category');
                    if ($prev_post): ?>
    ...
    <div class="container mx-auto px-6">
        <div class="max-w-6xl mx-auto">
            <div class="flex items-center justify-center flex-col text-center gap-4 mb-12">
                <div class="py-1 px-3 text-emerald-600 font-semibold border-b-2 border-emerald-600">
                    Related Industries
                </div>
                <h2 class="text-3xl font-bold text-foreground">Other Sectors We Empower</h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <?php while ($related_query->have_posts()): $related_query->the_post();
                    $rel_cats      = get_the_terms(get_the_ID(), 'industry_category');
                    $rel_cat_name  = (!is_wp_error($rel_cats) && !empty($rel_cats)) ? $rel_cats[0]->name : '';
                    $rel_icon      = get_post_meta(get_the_ID(), 'industry_icon', true);
                ?>
                <div class="group">
                    <div data-slot="card" class="text-card-foreground flex flex-col gap-6 rounded-xl shadow-sm h-full bg-background border border-border transition-all duration-500 p-7 relative overflow-hidden hover:shadow-lg hover:border-emerald-500">
                        <div data-slot="card-content" class="p-0">
                            <div class="flex items-start mb-5">
                                <div class="size-12 rounded-full flex items-center justify-center group-hover:scale-110 transition-all duration-500 bg-emerald-100/40 dark:bg-emerald-950/40 overflow-hidden">
                                    <?php if ($rel_icon): ?>
                                        <i data-lucide="<?php echo esc_attr($rel_icon); ?>"
                                           class="w-6 h-6 text-emerald-600 dark:text-emerald-400"></i>
                                    <?php else: ?>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-600 dark:text-emerald-400" aria-hidden="true">
                                            <path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"/><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"/><path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2"/><path d="M10 6h4"/><path d="M10 10h4"/><path d="M10 14h4"/><path d="M10 18h4"/>
                                        </svg>
                                    <?php endif; ?>
                                </div>
                            ...
                            <?php if ($rel_cat_name): ?>
                            <span class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-2 block"><?php echo esc_html($rel_cat_name); ?></span>
                            <?php endif; ?>
                            <h3 class="text-xl font-bold text-foreground mb-3 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors leading-tight">
                        ...
                        <div class="absolute bottom-0 left-0 w-full h-0.5 bg-linear-to-br transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left from-emerald-500 via-teal-600 to-teal-700"></div>
